...
function dateF() {
    ...
    for (var d = date1; d <= date2; d.setMonth(d.getMonth() + 1, 1)) {
        ...
        let daValue = Math.round((dutyPay * daPercentage) / 100);
    ...
    function slsCreate(year, daMonth, month) {
        if (data[ac] !== undefined) {
            ...
            if (daMonth === slsMonth && year === slsYear && slsDays !== 0) {
                ...
                let slsdutypayValue = Math.round((result * slsDays1) / 100);
                ...
                var slsGross = slsdutypayValue + slsppValue + slsdaValue + slshraValue + slsccaValue + slshaValue + slswaValue
